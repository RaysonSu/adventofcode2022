...
class State:
    ...
    def get_actions(self: State) -> list[int]:
        ...
        # e.g. [0, 2] would be build an ore bot (0) and build an obsidian bot (2)
        ret: list = []
...
def main() -> None:
    ...
    with multiprocessing.Pool(len(blueprints)) as pool:
        for index, result in enumerate(pool.map(deal_with_blueprint, blueprints)):
            ret += result * (index + 1)
