F = open("python/Advent of Code/2022/Day 9/input.txt", "r").readlines()
...
def move(direction):
    ...
    # tail logic
    x_diff = knots[0][0] - knots[1][0]
    y_diff = knots[0][1] - knots[1][1]
    ...
    if (abs(x_diff) > 1 or abs(y_diff) > 1) and abs(x_diff) > 0 and abs(y_diff) > 0:
        knots[1][0] += x_diff // abs(x_diff)
        knots[1][1] += y_diff // abs(y_diff)
